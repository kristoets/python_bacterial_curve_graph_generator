...
def growthCurve2(fileName):
    file_name = '/media/files/' + fileName  # Replace with the actual file name
    print(file_name)
    return os.listdir('/media/files/')

def growthCurve(fileName):
    # Read the Excel file
    file_name = '/media/files/' + fileName  # Replace with the actual file name
    print(file_name)
    ...
    graph_title_prefix = 'NaF concentration (mM) '  # Prefix for the graph title
    ...
    fig.canvas.draw()
    legend.set_bbox_to_anchor((0.85, 0.85))  # Position the legend in the center of the right edge of the figure
